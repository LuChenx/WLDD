	    	...
	    	<table class="table table-striped">
				  ...
				  <tbody>
				    <tr>
				      ...
				      <td>
				      <input style="float:right;width:350px;" type="text" class="form-control" id="address2" placeholder="请输入详细地址">
				      <div class="address">
                     	地址：<select id="area2"></select>  
						省：<select id="cmbProvince2"></select>  
						市：<select id="cmbCity2"></select>  
						区：<select id="cmbArea2"></select>
                     </div></td>
...
<script>
	...
	addressInit('area2','cmbProvince2','cmbCity2','cmbArea2','西北地区', '北京', '市辖区', '东城区'); 
	...
	$(function(){
		$("#createBtu").click(function(){
			if($("#name").val() == ""){
				...
				layer.msg('请输入寄件人详细地址', {icon: 5});
			}else if($("#phone").val() == ""){
				layer.msg('请输入收件人联系方式', {icon: 5});
			}else if($("#address2").val() == ""){
				layer.msg('请输入收件人详细地址', {icon: 5});
			}else{
				$.post("createOrderU.do",{itemType:$("#itemType").val(),orderEmType:$("#emType").val(),payType:$("#payType").val(),reviceName:$("#name").val(),
					revicePhone:$("#phone").val(),destination:$("#area2").val()+"|"+$("#cmbProvince2").val()+"|"+$("#cmbCity2").val()+"|"+
					$("#cmbArea2").val()+"|"+$("#address2").val(),address:$("#address").val()},function(data, textStatus){
						if(data.status){
